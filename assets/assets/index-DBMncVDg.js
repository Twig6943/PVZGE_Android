...
const gePatcher = {
  ...
  _gridItemFeaturesPath: "patches/jsons/features/GridItemFeatures.json",
  ...
  async initBase() {
    if (!await this._checkKeys()) return false;
    patchBaseDir = await window.electron.fs.getAppLocalDataPath();
    console.log("[GE Patcher] Base Dir: " + patchBaseDir);
    if (!window.cc) {
  ...
  async initPatchs() {
    ...
    await this._initFeatures(this._gridItemFeaturesPath, "GridItemFeatures", "GRIDITEMS");
    await this._initProps(this._gridItemPropsPath, "GridItemProps");
    await this._initProps(this._gridItemTypesPath, "GridItemTypes");
  ...
  _resolvePath(relativePath) {
    return `${patchBaseDir}/${relativePath}`.replace(/\\/g, "/");
  },
  async _initFeatures(featuresPath, featureType, featureKey) {
    const fullPath = this._resolvePath(featuresPath);
    if (await window.electron.fs.exists(fullPath)) {
      console.log(`[GE Patcher] Start loading ${featureType}...`);
      try {
        const modFeatures = await window.electron.fs.readTextFile(fullPath);
        this[`_mod${featureType}Data`] = lib.parse(modFeatures);
      } catch (e) {
        console.error(`[GE Patcher] Failed to load ${featureType}, error: ` + e);
        return;
      }
      const originalFeatures = this._jsonAssetsList.filter(((item) => typeof item === "object" && item._name == featureType))[0].json;
      ...
      if (originalFeatures && modFeaturesData[featureKey]) {
        for (const feature of modFeaturesData[featureKey]) {
          if (feature["CODENAME"]) {
            ...
            if (index !== -1) {
              originalFeatures[featureKey][index] = lodashExports.merge(originalFeatures[featureKey][index], feature);
            }
          }
        }
      }
      const additionalKeys = ["SEEDCHOOSERDEFAULTORDER", "BASEUNLOCKLIST"];
      for (const key2 of additionalKeys) {
        if (originalFeatures && modFeaturesData[key2]) {
          originalFeatures[key2] = modFeaturesData[key2];
        }
      }
  ...
  // ... (_initProps 和 setPropsData 的修改方式与 _initFeatures 类似) ...
  async _initProps(Path, Type) {
    const fullPath = this._resolvePath(Path);
    if (await window.electron.fs.exists(fullPath)) {
      console.log(`[GE Patcher] Start loading ${Type}...`);
      try {
        const modProps = await window.electron.fs.readTextFile(fullPath);
        this[`_mod${Type}Data`] = lib.parse(modProps);
      } catch (e) {
      ...
      if (originalProps && modPropsData["objects"]) {
        for (const obj of modPropsData["objects"]) {
          if (obj?.["aliases"]?.[0]) {
            ...
            if (index !== -1) {
              originalProps["objects"][index] = lodashExports.merge(originalProps["objects"][index], obj);
            }
          }
        }
      }
    ...
    if (originalProps?.["objects"]) {
      ...
      if (index !== -1 && originalProps["objects"][index].objdata) {
        if (args.length == 2 && typeof args[0] === "string") {
          ...
          originalProps["objects"][index].objdata = lodashExports.merge(originalProps["objects"][index].objdata, args[0]);
  ...
  async _initLevels() {
    this._levelList = this._jsonAssetsList.filter((e) => {
      return Array.isArray(e.json?.["objects"]) && e.json?.["objects"].find((e2) => e2.objclass == "LevelDefinition");
    });
    const levelsFullPath = this._resolvePath(this._levelsPath);
    if (await window.electron.fs.exists(levelsFullPath)) {
      let modLevelEntries;
      try {
        modLevelEntries = await window.electron.fs.readDir(levelsFullPath);
      ...
      for (const modLevelFile of modLevelEntries) {
        ...
        const modLevelPath = this._resolvePath(`${this._levelsPath}/${modLevelFile.name}`);
        console.log("[GE Patcher] Start loading level " + modLevelName + "...");
        try {
          const modLevelData = await window.electron.fs.readTextFile(modLevelPath);
          level.json = lib.parse(modLevelData);
  ...
  async help() {
    if (!await this._checkKeys()) return false;
    const appLocalDataDirPath = await window.electron.fs.getAppLocalDataPath();
    window.electron.ipcRenderer.send("e_openURL", "https://pvzge.com/en/guide/mod/");
    console.log("[GE Patcher] Base Dir: " + appLocalDataDirPath);
    console.log("[GE Patcher] PvZ2 Gardendless patcher system help");
    console.log("[GE Patcher] Patcher System Version: " + this.version);
    console.log("[GE Patcher] Warning: This is a beta version for gardendless 0.4, All features may be subject to significant changes in future versions!");
    console.log("[GE Patcher] Base Dir: " + appLocalDataDirPath + "\\patches");
    console.log("[GE Patcher] Put your patch files in the patches folder, see below for the paths");
    console.log("[GE Patcher] Paths:");
    console.log("[GE Patcher] - PlantFeatures: " + appLocalDataDirPath + "/" + this._plantFeaturesPath);
    console.log("[GE Patcher] - PlantAlmanac: " + appLocalDataDirPath + "/" + this._plantAlmanacPath);
    console.log("[GE Patcher] - PlantProps: " + appLocalDataDirPath + "/" + this._plantPropsPath);
    console.log("[GE Patcher] - ZombieFeatures: " + appLocalDataDirPath + "/" + this._zombieFeaturesPath);
    console.log("[GE Patcher] - ZombieAlmanac: " + appLocalDataDirPath + "/" + this._zombieAlmanacPath);
    console.log("[GE Patcher] - ZombieProps: " + appLocalDataDirPath + "/" + this._zombiePropsPath);
    console.log("[GE Patcher] - GridItemFeatures: " + appLocalDataDirPath + "/" + this._gridItemFeaturesPath);
    console.log("[GE Patcher] - GridItemProps: " + appLocalDataDirPath + "/" + this._gridItemPropsPath);
    console.log("[GE Patcher] - GridItemTypes: " + appLocalDataDirPath + "/" + this._gridItemTypesPath);
    console.log("[GE Patcher] - ProjectileProps: " + appLocalDataDirPath + "/" + this._projectilePropsPath);
    console.log("[GE Patcher] - ProjectileTypes: " + appLocalDataDirPath + "/" + this._projectileTypesPath);
    console.log("[GE Patcher] - UpgradeFeatures: " + appLocalDataDirPath + "/" + this._upgradeFeaturesPath);
    console.log("[GE Patcher] - PropertySheets: " + appLocalDataDirPath + "/" + this._propertySheetsPath);
    console.log("[GE Patcher] - NarrativeList: " + appLocalDataDirPath + "/" + this._narrativeListPath);
    console.log("[GE Patcher] - Store: " + appLocalDataDirPath + "/" + this._storePath);
    console.log("[GE Patcher] - Levels: " + appLocalDataDirPath + "/" + this._levelsPath);
    console.log("[GE Patcher] Usage:");
    console.log("[GE Patcher] - gePatcher.init() to start the patcher system");
    console.log("[GE Patcher]   - It will load all patch files automatically");
    console.log("[GE Patcher]   - You can also run gePatcher.initBase() first to load game assets, then run gePatcher.initPatchs() later to load patch files");
    console.log("[GE Patcher] - gePatcher.help() to show this help message");
    console.log("[GE Patcher] Below functions need to run after gePatcher.init() or gePatcher.initBase():");
    console.log("[GE Patcher] - gePatcher.showLevels() to list all levels names in the game");
    console.log("[GE Patcher] - gePatcher.setFrameRate(fps) to set the game frame rate (use it after enter your save)");
    console.log("[GE Patcher] - gePatcher.setPropsData(Type, aliase, key, value) to set a single property of a plant/zombie/griditem");
    console.log("[GE Patcher] - gePatcher.setPropsData(Type, aliase, {key1: value1, key2: value2}) to merge multiple properties of a plant/zombie/griditem");
    console.log("[GE Patcher]   - Supported Types: PlantProps, PlantAlmanac, ZombieProps, ZombieAlmanac, GridItemProps, GridItemTypes, PropertySheets, ProjectileProps, ProjectileTypes");
    console.log("[GE Patcher] Info:");
    console.log("[GE Patcher] - Put all your level jsons in Levels folder, and their names should be the same as the original level names");
  },
  // ... (其他函数 showLevels, setFrameRate 等保持不变) ...
  async showLevels() {
...
console.log("[GE Patcher] Patcher system is ready. Run gePatcher.init() to start.");
console.log("[GE Patcher] For patcher system help, run gePatcher.help()");
window.gePatcher = gePatcher;
(async () => {
  try {
    if (window.electron && window.electron.ipcRenderer) {
      window.electron.ipcRenderer.send("e_openURL", "https://pvzge.com/en/download/");
    }
  } catch (e) {
    console.error("Failed to initialize startup modules:", e);
  }
})();
